...
class PositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, warn=True, visualize=True):
        ...
        """
        """
        self.ghosts = gameState.getGhostPositions() # *** CAMBIOS (Ghosts) ***
...
Se agregan 2 clases nuevas para describir el nuevo problema asociado a la modificación:
- StaticGhostsPositionSearchProblem: Esta es la que se utiliza para comparar con A*, DFS y BFS 
(se adjunta la guía en el git).

- AStarStaticGhostsSearchAgent: Esta es sólo para usar A*, la cual se utilizó al comienzo de la
implementación. 

"""
class StaticGhostsPositionSearchProblem(search.SearchProblem):
    """
    A search problem defines the state space, start state, goal test, child
    function and cost function.  This search problem can be used to find paths
    to a particular point on the pacman board.
    ...
    def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, warn=True, visualize=True):
        ...
        """
        La posición de los fantasmas se agregan como nuevas murallas.
        """
...
class AStarStaticGhostsSearchAgent(SearchAgent):
    "A SearchAgent for FoodSearchProblem using A* and your foodHeuristic"
    def __init__(self):
        self.searchFunction = lambda prob: search.astar(prob, manhattanHeuristic)
        self.searchType = StaticGhostsPositionSearchProblem
...
def foodHeuristic(state, problem):
    """
    Heurística para el FoodSearchProblem

    En cada iteración se calcula la distancia actual entre pacman y todos los puntos (food). Para
    evaluar cómo va el algoritmo, consideraremos que la prioridad viene dada por la diferencia
    entre las distancias más largas, ya que es menor a la distancia que realmente el pacman debe
    recorrer para obtener ambos puntos, por lo que la heurística puede ser admisible.
    ...
    for pos in foodGrid.asList():
        distances.append( mazeDistance(position, pos, problem.startingGameState) )
    if len(distances) > 1:
